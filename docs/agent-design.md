...
| Agent | 输入 | 输出 | 职责 |
|-------|------|------|------|
| RequirementAgent | GenerationContext | List[Dict] | 理解需求，提取特性/功能点 |
| RagAgent | GenerationContext | RAGResultDTO | 从知识库检索相关上下文 |
| CaseGenerateAgent | GenerationContext | List[CaseDTO] | 根据需求和RAG上下文生成测试用例 |
| ReviewAgent | GenerationContext + List[CaseDTO] | ReviewResultDTO | 审查测试用例质量 |
| MindmapAgent | GenerationContext | MindmapDTO | 根据需求生成思维导图 |

### Agent规则

- 禁止直接操作数据库（通过Service层）
- 禁止直接发起HTTP请求（通过knowledge client）
- 禁止直接读写文件（通过upload service）
- 统一输入：GenerationContext，统一输出：DTO
