...
class LegacyExecutionRunner:
    ...
    async def run_execution(execution_id: int) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if not record:
                yield json.dumps({"step": "错误", "progress": 0, "message": "执行记录不存在"}, ensure_ascii=False)
                return
            ...
            if not script:
                ...
                db.commit()
                yield json.dumps({"step": "错误", "progress": 0, "message": "脚本不存在"}, ensure_ascii=False)
                return
            ...
            yield json.dumps({"step": "错误", "progress": 0, "message": f"读取数据失败: {e}"}, ensure_ascii=False)
            return
        ...
        try:
            ...
            if result:
                ...
                record.log_content = result.get("log_content")
                ...
                db.commit()
                yield json.dumps({"step": "执行异常", "progress": 100, "message": "执行结果为空"}, ensure_ascii=False)
        except Exception as e:
            err_msg = f"{type(e).__name__}: {e}" if str(e) else type(e).__name__
            log.error(f"更新执行记录失败 | exec={execution_id}: {err_msg}")
            yield json.dumps({"step": "执行异常", "progress": 100, "message": f"更新执行记录失败: {err_msg}"}, ensure_ascii=False)
        finally:
            db.close()
