...
export default function PublishPage({ onDetail }: Props) {
  ...
  const handleExecute = async () => {
    ...
    try {
      const res = await fetch('/api/assets/v2/execute', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
        body: JSON.stringify({ asset_ids: selectedRowKeys.map(Number) }),
      });
      const data = await res.json();
      if (data.summary) {
        message.success(`执行完成: 通过 ${data.summary.pass}, 失败 ${data.summary.fail}`);
        setSelectedRowKeys([]);
        fetchData();
      }
